...
function aircraftIcon(){return L.divIcon({className:"plane-marker",html:'<div class="plane-glyph">✈</div>',iconSize:[30,30],iconAnchor:[15,15]})}
...
function render(flights){
  ...
  for(const f of flights){
    ...
    const id=String(f.flight_id||f.callsign||("flight:"+lat+":"+lon));active.add(id);updateTrail(id,lat,lon);
    let marker=markers.get(id);
    if(!marker){
      marker=L.marker([lat,lon],{icon:planeIcon,keyboard:false});
      marker.bindTooltip(f.callsign||f.username||"Flight",{direction:"top",offset:[0,-10]});
      marker.addTo(map);markers.set(id,marker);
    }else animateMarker(marker,lat,lon);
    marker.off("click").on("click",()=>loadFlightDetail(f));rotateMarker(marker,f.heading_deg);
  }
  for(const [id,marker] of markers)if(!active.has(id)){
    map.removeLayer(marker);markers.delete(id);const line=trailLines.get(id);if(line)map.removeLayer(line);trailLines.delete(id);trailHistory.delete(id);
  }
}
...
async function showWikiPhoto(f){
  const query=[f.livery_name,f.aircraft_type,f.callsign].filter(Boolean).join(" ");if(!query)return;
  ...
  try{const d=await(await fetch(url,{cache:"no-store"})).json();const p=Object.values(d?.query?.pages||{})[0];if(p?.imageinfo?.[0])f.wiki_photo={url:p.imageinfo[0].thumburl||p.imageinfo[0].url,title:p.title,description:p.imageinfo[0].descriptionurl}}catch{}
}
function renderFlightDetails(f){
  $("details").className="";
  const aircraftName=f.aircraft_type||"Aircraft type unavailable",liveryName=f.livery_name||"Livery unavailable",dest=f.destination?.identifier||f.destination?.name||"Unknown",routeText=Array.isArray(f.route)&&f.route.length?f.route.length+" route points":"Route unavailable";
  $("details").innerHTML='<div class="card">'+(f.wiki_photo?'<img class="photo" src="'+escapeHtml(f.wiki_photo.url)+'" alt="Aircraft photo"><div class="photo-credit">Wikimedia Commons · '+escapeHtml(f.wiki_photo.title||"")+"</div>":"")+
  ...
  '<div class="grid"><div><div class="label">Pilot</div><div class="value">'+escapeHtml(f.username||"—")+'</div></div><div><div class="label">Server</div><div class="value">'+escapeHtml(selectedServer)+'</div></div>'+
  '<div><div class="label">Destination</div><div class="value"><span class="route-badge">'+escapeHtml(dest)+'</span></div></div><div><div class="label">Route</div><div class="value">'+escapeHtml(routeText)+'</div></div>'+
  ...
  '<div class="detail-actions"><button class="small-btn" id="routeBtn">Show route</button><button class="small-btn" id="focusBtn">Focus aircraft</button></div></div>';
  $("routeBtn").addEventListener("click",()=>drawSelectedRoute(f));$("focusBtn").addEventListener("click",()=>map.setView([Number(f.latitude),normLon(f.longitude)],8));
}
...
$("searchBtn").addEventListener("click",applySearch);$("resetBtn").addEventListener("click",()=>{$("search").value="";applySearch()});$("fitBtn").addEventListener("click",fitAircraft);$("refreshBtn").addEventListener("click",load);$("airportsToggle").addEventListener("click",toggleAirports);
